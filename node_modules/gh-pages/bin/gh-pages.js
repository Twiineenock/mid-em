...
import { publish as _publish, defaults } from '../lib/index.js';
...
function publish(dist, config) {
  return new Promise((resolve, reject) => {
    const basePath = _resolve(process.cwd(), dist);
    _publish(basePath, config, (err) => {
      if (err) {
        return reject(err);
      }
      resolve();
...
async function main(args) {
  ...
  const program = new Command()
    ...
    .option('--git <git>', 'Path to git executable', defaults.git)
    .option('-t, --dotfiles', 'Include dotfiles')
    .option('-r, --repo <repo>', 'URL of the repository you are pushing to')
    .option('-p, --depth <depth>', 'depth for clone', defaults.depth)
    .option(
      '-o, --remote <name>',
  ...
  let user;
  if (options.user) {
    const parts = parseOneAddress(options.user);
    if (!parts) {
      throw new Error(
  ...
  return await publish(options.dist, config);
}
